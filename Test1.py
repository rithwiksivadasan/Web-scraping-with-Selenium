...
# -*- coding: utf-8 -*-
"""
Created on Tue Dec 27 11:51:15 2022
...
for i in range (1,2):
    ...
    for i in range(0,len(list1)):
         if 'lastName' in list1[i]:
             ...
             key= str(re.findall('(?<=\"sports\":).*?(?=\,)',list1[i]))
             #print(key)
             special_characters=[']','[',"'"]
             for i in special_characters:
                 key=key.replace(i,"")
    ...
    for i in range(0,len(list1)):
         if 'lastName' in list1[i]:
             ...
             key= str(re.findall('(?<=\"accountIdentifier\":).*?(?=\,)',list1[i]))
             #print(key)
             special_characters=[']','[',"'"]
             for i in special_characters:
                 key=key.replace(i,"")
...
ath = {'first_name':first_name,'last_name':last_name,'current_team':current_team,'InstagramReach':instagram_reach,'twitterReach':twitter_reach,'instagramHandle':instagram_handle,'twitterHandle':twitter_handle,'biography':biography,'profileLink':profile_link,'Athlete_type':athlete_type,"Sport":sport}
